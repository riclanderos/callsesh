...
export default async function DashboardPage() {
  ...
  const [{ data: upcomingRaw }, { data: sessionTypes }, { count: usedCount }] = await Promise.all([
    supabase
      .from('bookings')
      .select('id, guest_name, booking_date, start_time, end_time, session_types(title)')
      .eq('coach_id', user.id)
      .eq('status', 'confirmed')
  ...
  return (
    <div className="min-h-screen px-6 py-10">
      <div className="mx-auto max-w-4xl space-y-8">
          ...
          <div className="space-y-1">
            ...
            {remaining <= 0 ? (
              <p className="text-xs font-medium text-amber-400">
                ...
                <a href="/upgrade" className="underline underline-offset-2 hover:text-amber-300 transition-colors">
                  Upgrade to continue
                </a>
              </p>
            ) : remaining <= 3 ? (
              ...
              <p className="text-xs text-zinc-500">{used} of {SESSION_LIMIT} sessions used</p>
            )}
          </div>
          <form action={logout}>
            <button
            ...
            <div className="rounded-xl border border-indigo-900 bg-zinc-800/60 p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1.5">
                  <p className="text-lg font-semibold text-zinc-100">{toTitleCase(next.guest_name)}</p>
              ...
              {rest.map((b) => (
                <div key={b.id} className="flex items-center justify-between px-5 py-3.5 gap-4">
                  <div className="space-y-0.5 min-w-0">
                    <p className="text-sm font-medium text-zinc-100">{toTitleCase(b.guest_name)}</p>
                    <p className="text-xs text-zinc-500">
                      {formatDate(b.booking_date)} · {formatTime(b.start_time)}
                    </p>
                  </div>
                  <span className="flex-shrink-0 text-xs text-zinc-500">{b.sessionTitle}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        {/* Booking links */}
        <section className="space-y-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Your booking links</p>
          {sessionTypes && sessionTypes.length > 0 ? (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 divide-y divide-zinc-800">
              {sessionTypes.map((st) => {
                const url = `${baseUrl}/book/${st.slug}`
                return (
                  <div key={st.id} className="flex items-center justify-between px-5 py-4 gap-4">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-zinc-100">{st.title}</p>
                      <p className="text-xs mt-0.5 truncate">
                        <span className="text-zinc-600">{baseUrl.replace(/^https?:\/\//, '')}</span>
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 px-5 py-6 text-center">
              <p className="text-sm text-zinc-500">
                ...
                <Link href="/dashboard/session-types" className="text-indigo-400 hover:text-indigo-300 transition-colors">
                  Create one →
                </Link>
              </p>
            </div>
        ...
        {/* Quick actions */}
        <section className="space-y-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Quick actions</p>
        ...
        {/* Manage */}
        <section className="space-y-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Manage</p>

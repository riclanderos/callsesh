...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    console.log(
      '[webhook] generated guest_access_token length:',
    ...
    if (insertError) {
      if ((insertError as { code?: string }).code === '23505') {
        ...
        try {
          ...
          console.error(
            'Daily room provisioning failed (retry path) for booking',
            existing.id,
            e,
          );
          return NextResponse.json(
            { error: 'Daily room provisioning failed.' },
            { status: 500 },
          );
        }
        return NextResponse.json({ received: true });
    ...
    console.log(
      '[webhook] post-insert guest_access_token:',
      newBooking
    ...
    if (newBooking) {
      try {
        ...
        console.error(
          'Daily room provisioning failed for booking',
          newBooking.id,
          e,
        );
        return NextResponse.json(
      ...
      console.error(
        'Could not resolve booking id after insert for session:',
        session.id,
      );
      return NextResponse.json(
        { error: 'Booking lookup failed after insert.' },
        { status: 500 },
      );
    }
    ...
    if (confirmedBooking?.status !== 'confirmed') {
      console.error(
        '[webhook] Booking not confirmed after insert — skipping email',
        {
    ...
    if (coachEmail) {
      ...
      console.error(
        'Could not resolve coach email for booking notification:',
        m.coach_id,
      );
    }
  }
  ...
  if (event.type === 'invoice.payment_succeeded') {
    ...
    if (subId) {
      ...
      if (error)
        console.error(
          '[webhook] invoice subscription update error:',
          error.message,
        );
  ...
  if (event.type === 'customer.subscription.updated') {
    ...
    console.log('Cancel at period end:', isCancelScheduled);
  ...
  if (event.type === 'subscription_schedule.updated') {
    ...
    if (subId) {
      ...
      const futurePriceIds = futurePhases.map((p) =>
        toPriceId(p.items[0]?.price),
      );
      ...
      console.log(
        '[webhook] subscription_schedule.updated — supabase update result',
        {
          event_id: event.id,
          subscription_id: subId,
          cancel_at_period_end: hasFuturePhase,
      ...
      console.warn(
        '[webhook] subscription_schedule.updated — no subscription_id resolved, skipping update',
        {
          event_id: event.id,
          schedule_id: schedule.id,
        },
